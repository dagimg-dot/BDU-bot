...
def display_final(message):
    ...
    users[message.from_user.id].rev_message_id = main_msg.message_id
...
def rotate_prediction(query):
    ...
    if query.data == "p":
        if users[query.from_user.id].current_state == 0:
            bot.answer_callback_query(query.id,"⚠️  Start of list  ⚠️",show_alert=True)
        else:
            ...
            bot.edit_message_text(full_str,users[query.from_user.id].user_id,users[query.from_user.id].rev_message_id,reply_markup=InlineKeyboardMarkup().row(prev_button,next_button))
